...
public class TwitterElasticIndexClient implements ElasticIndexClient<TwitterIndexModel> {
    ...
    public List<String> save(List<TwitterIndexModel> documents) {
        ...
        List<IndexedObjectInformation> indexedObjects = elasticsearchOperations.bulkIndex(
                indexQueries,
                IndexCoordinates.of(elasticConfigData.getIndexName())
        );

        List<String> documentIds = indexedObjects.stream()
                .map(doc -> new IndexedObjectInformation(doc.id(), doc.index(), doc.seqNo(), doc.primaryTerm(), doc.version()).id())
